...
def sim_input_data():
    ...
    for key in model_config.keys():
        ...
        theta_dict_all_scalars = {param: np.mean([model_config[key]['param_bounds'][0][i],
                                                  model_config[key]['param_bounds'][1][i]]) for i, param in enumerate(model_param_list)}
        # Dictionary with all vectors
        theta_dict_all_vectors = {param: np.tile(np.mean([model_config[key]['param_bounds'][0][i],
                                                    model_config[key]['param_bounds'][1][i]]), 100) for i, param in enumerate(model_param_list)}
